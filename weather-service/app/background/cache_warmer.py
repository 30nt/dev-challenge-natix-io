...
async def warm_cache(app):
    ...
    try:
        ...
        if remaining_tokens < settings.cache_warm_min_tokens_remaining:
            logger.info(f"Skipping cache warming: only {remaining_tokens} tokens remaining")
            return
        
        top_cities = await cache_service.get_top_cities(settings.top_cities_count)
        
        if not top_cities:
            top_cities = [(city, 0) for city in DEFAULT_CITIES[:settings.top_cities_count]]
        ...
        logger.info(f"Warming cache for {tokens_to_use} cities (of {len(top_cities)} top cities)")
