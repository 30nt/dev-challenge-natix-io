...
class Settings(BaseSettings):
    ...
    weather_api_url: str = os.getenv("WEATHER_API_URL", "https://api.example.com/weather")
    ...
    cache_warm_max_tokens: int = int(os.getenv("CACHE_WARM_MAX_TOKENS", "20"))  # Max 20% of rate limit
    cache_warm_min_tokens_remaining: int = int(os.getenv("CACHE_WARM_MIN_TOKENS_REMAINING", "50"))  # Keep 50 tokens for users

    circuit_breaker_failure_threshold: int = int(os.getenv("CIRCUIT_BREAKER_FAILURE_THRESHOLD", "5"))
    circuit_breaker_recovery_timeout: int = int(os.getenv("CIRCUIT_BREAKER_RECOVERY_TIMEOUT", "300"))  # 5 minutes
    circuit_breaker_expected_exception: Optional[str] = os.getenv("CIRCUIT_BREAKER_EXPECTED_EXCEPTION")
    ...
    cors_allow_methods: list[str] = os.getenv("CORS_ALLOW_METHODS", '["*"]').strip('"[]').split(",")
